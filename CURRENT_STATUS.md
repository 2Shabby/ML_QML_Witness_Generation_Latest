...
- `env/rocm/bin/python -m pytest -q` reports 117 passed and one failed test. `TestMLPDiscriminator.test_single_sample` fails because the model is left in training mode and `BatchNorm1d` rejects a batch containing one sample.
...
The current checkout result is 117/118 tests passing. This supersedes the former legacy statement that 115/118 tests pass.
...
4. The working local environment is installed, but a portable dependency lock or constraints file is absent.
5. CVXPY is installed locally but remains commented out in `requirements.txt`, although SDP tests require it.
6. The near-perfect nonlinear results have not yet been documented with leakage and family-held-out controls.
7. PennyLane is installed locally and a smoke check passed, but no reusable quantum circuit, classifier, training pipeline, test, or normalization-control experiment exists in the repository.
8. The MLP's batch-normalization path fails for a single sample while the model is in training mode.

## Current research priorities

1. Resolve the single-sample MLP/BatchNorm failure and restore a fully passing test suite.
2. Record portable dependency constraints or a lock strategy, including the ROCm-specific Torch installation and a CPU fallback.
3. Reproduce the current classical manuscript table and commit result artifacts.
4. Freeze seeds and train/test indices so every model uses identical splits.
5. Run raw, L2-normalized, and L2-normalized-plus-norm classical controls.
6. Stress-test MLP and transformer performance under family-held-out and boundary-focused evaluation.
7. Repeat the 36D-vs-63D comparison for nonlinear models.
8. Implement the proposed six-qubit amplitude-encoded classifier separately from direct-state input experiments.
