...
\begin{enumerate}[noitemsep]
    \item \textbf{Demonstration} that 36-dimensional restricted features (1+2 body Paulis) achieve $>85\%$ classification accuracy
    \item \textbf{Evidence} that three-body correlations are not essential: 36D matches or exceeds 63D performance
    \item \textbf{Comparison} of linear (SVM) and non-linear (Transformer) models, achieving up to 100\% accuracy
...
\begin{claim}
Linear SVM achieves $>85\%$ accuracy on restricted 36D features.
\end{claim}

\begin{table}[h]
\centering
\caption{Model comparison (5,000 samples, seed=42)}
\begin{tabular}{lcccc}
\toprule
...
Transformer Hybrid & \textbf{100.0\%} & 100.0\% & 100.0\% & 100.0\% \\
\bottomrule
\end{tabular}
\end{table}
...
Both models produce 36-term witnesses, but with fundamentally different structures:
...
\begin{enumerate}
    ...
    \item \textbf{Multiple model options:} Linear SVM (85.6\%, interpretable) vs.\ Transformer (99.7--100\%, higher accuracy)
...
python scripts/run_experiments.py \
    --experiment ablation --n-samples 5000 --seed 42
